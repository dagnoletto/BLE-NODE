...
uint8_t HCI_Disconnect( uint16_t Connection_Handle, CONTROLLER_ERROR_CODES Reason, DefCmdStatus StatusCallBack )
{
	if( Connection_Handle <= MAX_CONNECTION_HANDLE )
	{
		...
		TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_DISCONNECT );
		...
		if( TxDesc.DataPtr != NULL )
		{
			...
			TxDesc.DataPtr->Size = sizeof(HCI_SERIAL_COMMAND_PCKT) + 3;

			HCI_SERIAL_COMMAND_PCKT* PcktPtr = (typeof(PcktPtr))( &TxDesc.DataPtr->Bytes[0] );
			...
			return ( HCI_Transmit_Command( &TxDesc, NULL, StatusCallBack ) );
		...
		ReadRemoteVerInfoComplete CompleteCallBack, DefCmdStatus StatusCallBack )
{
	...
	{
		TRANSFER_DESCRIPTOR TxDesc;
		TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_READ_REMOTE_VERSION_INFORMATION );
		...
		if( TxDesc.DataPtr != NULL )
		{
			...
			TxDesc.CallBack = NULL;
			TxDesc.CallBackMode = CALL_BACK_AFTER_TRANSFER;
			TxDesc.DataPtr->Size = sizeof(HCI_SERIAL_COMMAND_PCKT) + 2;
			...
			return ( HCI_Transmit_Command( &TxDesc, CompleteCallBack, StatusCallBack ) );
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_SET_EVENT_MASK /*, CompleteCallBack, StatusCallBack */ );

	if( TxDesc.DataPtr != NULL )
	{
		TxDesc.CallBack = NULL;
		TxDesc.CallBackMode = CALL_BACK_AFTER_TRANSFER;
		TxDesc.DataPtr->Size = sizeof(HCI_SERIAL_COMMAND_PCKT) + sizeof(EVENT_MASK);
		...
		TxPwrLvlComplete CompleteCallBack, DefCmdStatus StatusCallBack )
{
	if( ( Connection_Handle <= MAX_CONNECTION_HANDLE ) && ( Type <= 1 ) )
	{
		...
		TRANSFER_DESCRIPTOR TxDesc;
		TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_READ_TRANSMIT_POWER_LEVEL );
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_RESET );

	if( TxDesc.DataPtr != NULL )
	{
		TxDesc.CallBack = NULL;
		TxDesc.CallBackMode = CALL_BACK_AFTER_TRANSFER;
		TxDesc.DataPtr->Size = sizeof(HCI_SERIAL_COMMAND_PCKT);
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_READ_LOCAL_VERSION_INFORMATION );
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_READ_LOCAL_SUPPORTED_COMMANDS );
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_READ_LOCAL_SUPPORTED_FEATURES );
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_READ_BD_ADDR );
...
uint8_t HCI_Read_RSSI( uint16_t Handle, ReadRSSIComplete CompleteCallBack, DefCmdStatus StatusCallBack )
{
	...
	{
		TRANSFER_DESCRIPTOR TxDesc;
		TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_READ_RSSI );
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_SET_EVENT_MASK );

	if( TxDesc.DataPtr != NULL )
	{
		TxDesc.CallBack = NULL;
		TxDesc.CallBackMode = CALL_BACK_AFTER_TRANSFER;
		TxDesc.DataPtr->Size = sizeof(HCI_SERIAL_COMMAND_PCKT) + sizeof(LE_EVENT_MASK);
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_READ_BUFFER_SIZE );
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_READ_LOCAL_SUPPORTED_FEATURES );
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_SET_RANDOM_ADDRESS );

	if( TxDesc.DataPtr != NULL )
	{
		TxDesc.CallBack = NULL;
		TxDesc.CallBackMode = CALL_BACK_AFTER_TRANSFER;
		TxDesc.DataPtr->Size = sizeof(HCI_SERIAL_COMMAND_PCKT) + sizeof(Random_Address);
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_SET_ADVERTISING_PARAMETERS );

	if( TxDesc.DataPtr != NULL )
	{
		TxDesc.CallBack = NULL;
		TxDesc.CallBackMode = CALL_BACK_AFTER_TRANSFER;
		TxDesc.DataPtr->Size = sizeof(HCI_SERIAL_COMMAND_PCKT) + 15;
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_READ_ADV_PHY_CHANNEL_TX_POWER );
		...
		DefCmdComplete CompleteCallBack, DefCmdStatus StatusCallBack )
{
	...
	{
		TRANSFER_DESCRIPTOR TxDesc;
		TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_SET_ADVERTISING_DATA );

		if( TxDesc.DataPtr != NULL )
		{
			TxDesc.CallBack = NULL;
			TxDesc.CallBackMode = CALL_BACK_AFTER_TRANSFER;
			TxDesc.DataPtr->Size = sizeof(HCI_SERIAL_COMMAND_PCKT) + MAX_ADVERTISING_DATA_LENGTH + 1;
		...
		DefCmdComplete CompleteCallBack, DefCmdStatus StatusCallBack )
{
	...
	{
		TRANSFER_DESCRIPTOR TxDesc;
		TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_SET_SCAN_RESPONSE_DATA );

		if( TxDesc.DataPtr != NULL )
		{
			TxDesc.CallBack = NULL;
			TxDesc.CallBackMode = CALL_BACK_AFTER_TRANSFER;
			TxDesc.DataPtr->Size = sizeof(HCI_SERIAL_COMMAND_PCKT) + MAX_SCAN_RESPONSE_DATA_LENGTH + 1;
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_SET_ADVERTISING_ENABLE );

	if( TxDesc.DataPtr != NULL )
	{
		TxDesc.CallBack = NULL;
		TxDesc.CallBackMode = CALL_BACK_AFTER_TRANSFER;
		TxDesc.DataPtr->Size = sizeof(HCI_SERIAL_COMMAND_PCKT) + sizeof(Advertising_Enable);
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_SET_SCAN_PARAMETERS );

	if( TxDesc.DataPtr != NULL )
	{
		TxDesc.CallBack = NULL;
		TxDesc.CallBackMode = CALL_BACK_AFTER_TRANSFER;
		TxDesc.DataPtr->Size = sizeof(HCI_SERIAL_COMMAND_PCKT) + 7;
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_SET_SCAN_ENABLE );
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_CREATE_CONNECTION );

	if( TxDesc.DataPtr != NULL )
	{
		TxDesc.CallBack = NULL;
		TxDesc.CallBackMode = CALL_BACK_AFTER_TRANSFER;
		TxDesc.DataPtr->Size = sizeof(HCI_SERIAL_COMMAND_PCKT) + 25;
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_CREATE_CONNECTION_CANCEL );
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_READ_WHITE_LIST_SIZE );
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_CLEAR_WHITE_LIST );
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_ADD_DEVICE_TO_WHITE_LIST );
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_REMOVE_DEVICE_FROM_WHITE_LIST );
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_SET_HOST_CHANNEL_CLASSIFICATION );

	if( TxDesc.DataPtr != NULL )
	{
		TxDesc.CallBack = NULL;
		TxDesc.CallBackMode = CALL_BACK_AFTER_TRANSFER;
		TxDesc.DataPtr->Size = sizeof(HCI_SERIAL_COMMAND_PCKT) + sizeof(Channel_Map);
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_CONNECTION_UPDATE );

	if( TxDesc.DataPtr != NULL )
	{
		TxDesc.CallBack = NULL;
		TxDesc.CallBackMode = CALL_BACK_AFTER_TRANSFER;
		TxDesc.DataPtr->Size = sizeof(HCI_SERIAL_COMMAND_PCKT) + 14;
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_READ_CHANNEL_MAP );
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_READ_REMOTE_FEATURES );
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_ENCRYPT );

	if( TxDesc.DataPtr != NULL )
	{
		TxDesc.CallBack = NULL;
		TxDesc.CallBackMode = CALL_BACK_AFTER_TRANSFER;
		TxDesc.DataPtr->Size = sizeof(HCI_SERIAL_COMMAND_PCKT) + 32;
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_RAND );
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_ENABLE_ENCRYPTION );

	if( TxDesc.DataPtr != NULL )
	{
		TxDesc.CallBack = NULL;
		TxDesc.CallBackMode = CALL_BACK_AFTER_TRANSFER;
		TxDesc.DataPtr->Size = sizeof(HCI_SERIAL_COMMAND_PCKT) + 28;
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_LONG_TERM_KEY_REQUEST_REPLY );

	if( TxDesc.DataPtr != NULL )
	{
		TxDesc.CallBack = NULL;
		TxDesc.CallBackMode = CALL_BACK_AFTER_TRANSFER;
		TxDesc.DataPtr->Size = sizeof(HCI_SERIAL_COMMAND_PCKT) + 18;
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_LONG_TERM_KEY_RQT_NEG_REPLY );
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_READ_SUPPORTED_STATES );
...
uint8_t HCI_LE_Receiver_Test_v1( uint8_t RX_Channel, DefCmdComplete CompleteCallBack, DefCmdStatus StatusCallBack )
{
	...
	{
		TRANSFER_DESCRIPTOR TxDesc;
		TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_RECEIVER_TEST_V1 );

		if( TxDesc.DataPtr != NULL )
		{
			TxDesc.CallBack = NULL;
			TxDesc.CallBackMode = CALL_BACK_AFTER_TRANSFER;
			TxDesc.DataPtr->Size = sizeof(HCI_SERIAL_COMMAND_PCKT) + 1;
		...
		DefCmdComplete CompleteCallBack, DefCmdStatus StatusCallBack )
{
	...
	{
		TRANSFER_DESCRIPTOR TxDesc;
		TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_TRANSMITTER_TEST_V1 );
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_TEST_END );
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_ADD_DEVICE_TO_RESOLVING_LIST );

	if( TxDesc.DataPtr != NULL )
	{
		TxDesc.CallBack = &Hosted_LE_Add_Device_To_Resolving_List;
		TxDesc.CallBackMode = CALL_BACK_AFTER_TRANSFER;
		TxDesc.DataPtr->Size = sizeof(HCI_SERIAL_COMMAND_PCKT) + 39;
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_REMOVE_DEVICE_FROM_RESOLVING_LIST );
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_CLEAR_RESOLVING_LIST );
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_READ_RESOLVING_LIST_SIZE );
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_READ_PEER_RESOLVABLE_ADDRESS );
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_READ_LOCAL_RESOLVABLE_ADDRESS );
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_SET_ADDRESS_RESOLUTION_ENABLE );
...
{
	TRANSFER_DESCRIPTOR TxDesc;
	TxDesc.DataPtr = HCI_Get_Command_Transmit_Buffer_Free( HCI_LE_SET_RESOLVABLE_PRIVATE_ADDRESS_TIMEOUT );
...
uint8_t HCI_Host_ACL_Data( HCI_ACL_DATA_PCKT_HEADER* ACLDataPacketHeader, uint8_t Data[] )
{
	if( ACLDataPacketHeader->Data_Total_Length <= 27 )
	{
		...
		TxDesc.DataPtr = HCI_Get_Data_Transmit_Buffer_Free( );

		if( TxDesc.DataPtr != NULL )
		{
			TxDesc.CallBack = NULL;
			TxDesc.CallBackMode = CALL_BACK_AFTER_TRANSFER;
			TxDesc.DataPtr->Size = sizeof(HCI_SERIAL_ACL_DATA_PCKT) + ACLDataPacketHeader->Data_Total_Length;

			HCI_SERIAL_ACL_DATA_PCKT* PcktPtr = (typeof(PcktPtr))( &TxDesc.DataPtr->Bytes[0] );
			...
			return ( HCI_Transmit_Data( &TxDesc ) );
		}
	}
